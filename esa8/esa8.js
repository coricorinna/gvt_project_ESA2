var app = (function () {
    ...
    /**
     * Init pipeline parameters that will not change again. If projection or
     * viewport change, their setup must be in render function.
     */
    ...
    function initShaderProgram() {
        var selection = new URL(window.location.href).searchParams.get("ea8");
        if (["Toon", "Erweiterung"].includes(selection)) {
            var vertexshaderid = "vertexshadertoon";
        } else {
            var vertexshaderid = "vertexshader";
        }

        // Init vertex shader.
        var vs = initShader(gl.VERTEX_SHADER, vertexshaderid);
